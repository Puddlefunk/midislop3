<script lang="ts">
  ...
  // Module (panel) scale
  const PANEL_SCALE_KEY = 'panelScale';
  let panelScale = $state(parseFloat(localStorage.getItem(PANEL_SCALE_KEY) ?? '1'));
  $effect(() => {
    document.documentElement.style.setProperty('--panel-scale', String(panelScale));
  });

  function onPanelScaleInput(e: Event) {
    panelScale = parseFloat((e.target as HTMLInputElement).value);
    localStorage.setItem(PANEL_SCALE_KEY, String(panelScale));
  }
  ...
  function save() {
    const saveState = (window as Window & { saveState?: () => void }).saveState;
    saveState?.();
  }
...
<div id="menu-panel">
  ...
  <div class="mp-pane">
    ...
    <div class="mp-section">
      <div class="mp-section-label">MODULE SCALE</div>
      <div class="mp-scale-row">
        <input type="range" min="0.5" max="2" step="0.05" value={panelScale} oninput={onPanelScaleInput} />
        <span class="mp-scale-val">{Math.round(panelScale * 100)}%</span>
      </div>
    </div>
