<script lang="ts">
  ...
  onMount(() => {
    ...
    // Canvas width/height attribute change clears the canvas; $effect doesn't
    // re-fire synchronously after a state change inside onMount, so force a
    // draw once the new size has been applied.
    setTimeout(() => {
    ...
    // Re-measure whenever the control wrapper resizes so all panels stay
    // consistent if --fs-panel ever changes.
    const ro = new ResizeObserver(measureEm);
...
<div class="synth-control" bind:this={controlEl}>
  <label>{label}</label>
