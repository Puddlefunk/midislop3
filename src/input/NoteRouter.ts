...
export class NoteRouter {
  ...
  disconnectDevice(id: string): void {
    ...
    const mod = [...this.registry.modules.values()]
      .find(m => getRuntimeType(m.type) === 'midi-in' && m.params.deviceId === id);
  ...
  noteOn(midi: number, velocity: number, deviceId: string | null = null): void {
    const allMods = [...this.registry.modules.values()]
      .filter(m => getRuntimeType(m.type) === 'midi-all');
    for (const allMod of allMods) {
    ...
    if (deviceId) {
      const devMods = [...this.registry.modules.values()]
        .filter(m => getRuntimeType(m.type) === 'midi-in' && m.params.deviceId === deviceId);
      for (const devMod of devMods) {
  ...
  noteOff(midi: number, deviceId: string | null = null): void {
    ...
    if (!allMods.length) this._fire('off', { midi, velocity: 0, deviceId, generatorId: null });
  }
