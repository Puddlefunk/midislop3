<script lang="ts">
  ...
  let { gameState, registry, gameEngine, patchSystem, router, audioGraph }: {
    gameState:   GameState;
  ...
  let hoveredChain   = $state<Set<string>>(new Set());

  function chainFrom(rootId: string): Set<string> {
    ...
    while (queue.length) {
      ...
      visited.add(id);
      for (const p of registry.patchesFrom(id)) if (!visited.has(p.toId))   queue.push(p.toId);
      for (const p of registry.patchesTo(id))   if (!visited.has(p.fromId)) queue.push(p.fromId);
    }
    return visited;
  }
  ...
  onMount(() => {
    ...
    window.addEventListener('toggle-shop', () => { if (shopUnlocked) shopOpen = !shopOpen; });
    ...
    if (panelsContainer) {
      ...
      panelsContainer.addEventListener('chainhover', (e: Event) => {
        const { id, active } = (e as CustomEvent<{ id: string; active: boolean }>).detail;
        hoveredChain = active ? chainFrom(id) : new Set();
      });
...
<div id="ui-root">
    ...
    {#each modules as mod (mod.id)}
      ...
      {#if def}
        <ModulePanel
          ...
          chainHovered={hoveredChain.has(mod.id)}
        >
          {#snippet children()}
            {#if mod.type === 'vcf-x2' || def.panelKind === 'filter'}
              <FilterPanel modId={mod.id} type={mod.type} params={mod.params} />
            {:else if mod.type === 'chord' || def.panelKind === 'chord'}
              <ChordPanel modId={mod.id} type={mod.type} params={mod.params} />
            {:else if mod.type === 'mixer' || def.panelKind === 'mixer'}
              <MixerPanel modId={mod.id} type={mod.type} params={mod.params} />
            {:else if mod.type === 'noteSeq' || def.panelKind === 'noteSeq'}
              <NoteSeqPanel modId={mod.id} type={mod.type} params={mod.params} />
            {:else if mod.type === 'drumSeq' || def.panelKind === 'drumSeq'}
  ...
  <ShopPanel
    ...
    {router}
    {audioGraph}
    onSave={() => (window as Window & { saveState?: () => void }).saveState?.()}
  />
  ...
  <div id="game-controls">
    ...
    <button id="shop-btn" class:locked={!shopUnlocked} class:active={shopOpen} title="Unlocks at Level 2"
      onclick={() => { if (shopUnlocked) shopOpen = !shopOpen; }}>SHOP</button>
  </div>
